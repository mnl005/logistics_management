...
public class board_controller {
    ...
    public ResponseEntity<JsonNode> me(@RequestBody Dto<Object, Object> dto, HttpServletRequest request){
        dto = board_business.me(dto, request);
        return ResponseEntity.ok(mapper.valueToTree(dto));
    }

    // 게시글 등록
    @PostMapping("/insert")
    public ResponseEntity<JsonNode> insert(@RequestBody Dto<board_model, Object> dto, HttpServletRequest request){
        dto = board_business.insert(dto, request);
        return ResponseEntity.ok(mapper.valueToTree(dto));
    }
