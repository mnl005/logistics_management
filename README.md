...
+ 구성 
  * 언어 : JAVA 21, html, css, javascript, 제이쿼리
  * 프레임워크 : Spring Boot 3
  * 데이터베이스 : MySQL 8
  * 빌드도구 : Maven, Gradle
...
+ error : 에러 발생시 작동할 동작을 정의
+ id_data : 사용자가 클릭한 요소를 구분짓는 값, 예를 들어 게시글에서 여러 게시글 중 하나를 특정할 수 있는 값
+ msg : 서버에서 전송받는 메시지
+ req_data : 보낼 데이터
+ res_data : 받을 데이터
...
+ req_data,res_data의 데이터중 값이 문자열 길이가 100 이상인 경우 "많은값" 이라고 별도 표기
+ 에러발생시 메소드,클래스,메시지만 로그로 표시
